...
import { createCustomer, createPaymentIntent, handleWebhookEvent } from '../services/stripe.service.js';
import User from '../models/user.model.js'; // To find user details
import { protectRoute } from '../middleware/auth.middleware.js'; // To protect routes

const router = express.Router();

// Endpoint to create a payment intent
router.post('/create-payment-intent', protectRoute, async (req, res) => {
  const { planId } = req.body; // We'll define plan details later, for now, assume a fixed amount
  ...
  try {
    ...
    // For now, let's assume planId "basic" is $5 (500 cents) and "premium" is $10 (1000 cents)
    // This should be more robustly defined, perhaps in a config file or separate model
...
// Stripe requires the raw body for signature verification, so we use express.raw middleware
    ...
    switch (event.type) {
      case 'payment_intent.succeeded':
        ...
        if (user) {
          ...
          console.log('Webhook received for unknown customer:', paymentIntent.customer);
        }
        break;
